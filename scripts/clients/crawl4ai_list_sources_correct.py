#!/usr/bin/env python3
"""
List sources from Crawl4AI MCP server using the correct IP address from mcp.json.
"""
import asyncio
import json
import logging
...
async def list_crawl4ai_sources():
    ...
    # Use the correct IP address from your mcp.json configuration
    base_url = "http://192.168.50.7:8052"
    endpoint = "/messages/"
    ...
    try:
        ...
        logger.info("Using IP address from mcp.json configuration: 192.168.50.7:8052")
...
def format_sources_output(result, tool_names):
    ...
    print(f"Server: http://192.168.50.7:8052")
    print("=" * 70)
    ...
    print("   • Extract content: crawl4ai extract_content <url>")
    print("   • Crawl website: crawl4ai crawl_website <url>")  
    print("   • Search web: crawl4ai search_web <query>")
    print("   • Health check: crawl4ai health_check")
    print("=" * 70)

async def main():
    """Main function."""
    print("Crawl4AI MCP Server - List Available Sources")
    print("Configuration: Using IP 192.168.50.7:8052 from mcp.json")
    print("=" * 50)
    ...
    # Show raw result for debugging if requested
